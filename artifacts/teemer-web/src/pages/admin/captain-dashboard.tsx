...
const CAPTAIN_ACTIONS = [
  ...
  { status: "complete", label: "Finish Job", icon: CheckCircle, color: "bg-green-500 hover:bg-green-600" },
        ...
        <div className="border-t border-slate-100 p-4 space-y-4">
          {job.quoteData && (
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              {job.quoteData.estimatedHours != null && (
                <div className="text-slate-600">
                  <span className="text-xs text-slate-400 block">Est. Hours</span>
                  ~{job.quoteData.estimatedHours}h
                </div>
              )}
...
export default function CaptainDashboard() {
  ...
  const activeJobs = jobs.filter((j) => {
    ...
    return ["en_route", "arrived", "in_progress", "at_storage", "returning", "delayed"].includes(status);
  });
