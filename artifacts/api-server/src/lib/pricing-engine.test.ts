...
describe("Materials pricing", () => {
  test("Stretch wrap: 1 roll/bed when lightly furnished × $55", () => {
    const r = calculatePricing({
      ...base,
      numberOfBedrooms: 2,
      numberOfLivingRooms: 1,
    ...
    assert.equal(r.breakdown.stretchWrapCost, 2 * 1 * 55);
  });

  test("Stretch wrap: 2 rolls/bed when fully furnished × $55", () => {
    const r = calculatePricing({
      ...base,
      numberOfBedrooms: 2,
      numberOfLivingRooms: 1,
    ...
    assert.equal(r.breakdown.stretchWrapCost, 2 * 2 * 55);
  });
  ...
  test("Studio (0 bedrooms) with materials → 1 roll minimum (not 0)", () => {
    ...
    assert.equal(r.breakdown.stretchWrapCost, 1 * 2 * 55);
  });
  ...
  test("Small boxes $3.50 each, medium $6.50 each", () => {
    const r = calculatePricing({
      ...base,
      numberOfBedrooms: 1,
      numberOfLivingRooms: 1,
    ...
    assert.equal(r.breakdown.smallBoxCost, 10 * 3.5);
    assert.equal(r.breakdown.mediumBoxCost, 4 * 6.5);
  });
});
